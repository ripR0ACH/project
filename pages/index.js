...
countEvents(*screensize): 
  ...
  - the inner circle can contain a maximum number of 
    ...
    - Each size of screen will be a different class 
      for the circle, so that it renders differently on
      every screen size
...
animateEvents(*event_count, events)
  ...
  - events will animate to screen, first starting with
    ...
    circle
    animateInnerCircle
  loadInnerEvents(*event_count)
    will decide what events should be featured in the 
    inner circle
    - inner events will be based on the user's interest
      if they are logged into their account
  loadOuterEvents(*event_count)
    will decide what events should be featured in the outer
    section
    - outer events will be the same, but will have a
      larger radius for the distance to the event from
      the user

changeEventDistanceFilter(*current_location)
